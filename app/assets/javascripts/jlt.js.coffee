...
jcall = (vars, command) ->
  ...
  if vars[first]?
    ...
    if typeof window[first] == "function"
      result = window[first]()
    else
      result = window[first]
...
jQuery.fn.jlt = (source_data) ->
  ...
  @each ->
    ...
    for vars in data_array
      ...
      # e.g. <option jattrif=selected.male.gender> -> <option selected="selected">
      $clone.find("[jattrif]").each(->
        $e = $(this)
        segments = $e.attr('jattrif').split(/\./)
        attr = segments.shift()
        match = segments.shift()
        if match == jcalc(vars, segments.join("."))
          $e.attr attr, attr
      ).removeAttr 'jattrif'
